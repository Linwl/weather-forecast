    ...
    try {
      log.info(
          MessageFormat.format(
              "{0}接收到用户<{1}>的邮件<{2}>发送任务!", worker, recipient.getName(), recipient.getEmail()));
      // TODO:获取天气信息
      WeatherEntity weather =
          HttpUtil.syncGet(HttpUtil.getWeatherUrl(recipient.getCityId()), WeatherEntity.class);
      recipient.setWeather(Common.getContent(weather));
      EmailService emailService = EmailService.getInstance();
      emailService.send(
          recipient.getEmail(),
          MessageFormat.format(
              "{0}的天气预报信息", LocalDate.now().format(DateTimeFormatter.ofPattern("yyyy-MM-dd"))),
          recipient.getWeather());
      log.info(
          MessageFormat.format(
              "{0}执行完用户<{1}>的邮件<{2}>发送任务!", worker, recipient.getName(), recipient.getEmail()));
    } catch (Exception e) {
